...
public class ProductSpecsService {
    ...
    @AdminAuth(name = "添加产品规格", orderNum = 2)
    @ExplainOperation(name = "添加产品规格", notes = "添加产品规格信息", params = {
    ...
    @AdminAuth(name = "修改产品规格", orderNum = 3)
    @ExplainOperation(name = "修改产品规格", notes = "修改产品规格信息", params = {
            @ExplainParam(value = "id", name = "产品规格id", require = true, type = "int", example = "1"),
            @ExplainParam(value = "...", name = "其他信息", type = "Object", example = "对应其他数据")
    }, back = {
    ...
    public JsonResult update(String params) {
        try {
            ProductSpecs o = JSONObject.toJavaObject(JSON.parseObject(params), ProductSpecs.class);
            ProductSpecs obj = productSpecsDao.findOne(o.getId());
